...
(rf/reg-event-fx
 ...
 (fn [{:keys [db]} [new-traces]]
   (if-let [sorted-traces (sort-by :id new-traces)]
     (let [previous-traces (get-in db [:traces :all] [])
           total-trace-count (+ (count previous-traces) (count sorted-traces))
           _ (when (> total-trace-count throttle/default-buffer-size)
               (throttle/increment-drop-count!))
           _ (when (> total-trace-count throttle/default-buffer-size)
               (throttle/increment-buffer-size!))
           should-throttle? (or (> total-trace-count throttle/default-buffer-size)
                                (throttle/is-throttling?))]
       (if should-throttle?
         {:db (update db :traces (fn [traces]
                                   (let [current (or (:all traces) [])
                                         new-count (+ (count current) (count sorted-traces))]
                                     (if (> new-count throttle/default-buffer-size)
                                       (do (throttle/set-throttling! true)
                                           traces)
                                       (assoc traces :all (into current sorted-traces))))))}
         (let [number-of-epochs-to-retain (get-in db [:settings :number-of-epochs])
               ...
               parse-state                (assoc parse-state :partitions [])
